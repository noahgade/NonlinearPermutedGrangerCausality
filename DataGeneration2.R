...
data_sim <- function(Time, dims = c(6, 6), columns) {
  ...
  dat <- replicate(nsims, cbind(simTAR2(Time, dims[1]), simTAR2(Time, dims[2])), simplify = FALSE)
...
col_select1000b <- gen_sample(nsims)
save(col_select1000b, file = "cs1000b.RData")

# Data Generation
simdata1000b <- data_sim(1000 + max_lag, dims, col_select1000b)
save(simdata1000b, file = "simdata1000b.RData")
