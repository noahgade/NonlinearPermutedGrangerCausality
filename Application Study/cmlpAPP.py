...
data = np.asmatrix(mat[((mat[:,0] >= 11) & (mat[:,0] < 15)), 1:5])

dat = torch.tensor(((data - np.mean(data, axis = 0)) / np.sqrt(np.var(data, axis = 0)))[np.newaxis], dtype=torch.float32)
dimension = data.shape[-1]
cmlp0 = cMLP(dimension, lag = 200, hidden = [200], activation = "tanh")
train_model_ista(cmlp0, dat, lam = 0.5, lam_ridge = 1e-2, lr = 1e-3, penalty = 'H', max_iter = 20000, check_every = 100)
