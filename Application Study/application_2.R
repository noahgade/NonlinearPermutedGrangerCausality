...
datprep <- function(data, x_stimulus_col, z_stimulus_col) {
  out <- vector("list", 2)
  nr0 <- nrow(data)
  xkeep <- c(sapply(seq(25, 50, 5) * 4000 / 1000, function(x) -2:2 + x))
  zkeep <- c(sapply(seq(25, 50, 5) * 4000 / 1000, function(x) -2:2 + x), sapply(seq(25, 50, 5) * 4000 / 1000, function(x) -2:2 + x) + 220)
  ...
  for(col in 1:length(out)) {
    ...
    out[[col]]$X <- lagmatCpp(data[, x_stimulus_col, drop = FALSE], 220)[, xkeep, drop = FALSE]
    out[[col]]$Z <- lagmatCpp(data[, z_stimulus_col, drop = FALSE], 220)[, zkeep, drop = FALSE]
  }
  return(out)
}
...
# [1] 466
set.seed(466)
test_data <- datprep(dat, x_stimulus_col = 3, z_stimulus_col = c(2, 4))
...
save(appresult, file = "/home/ndg5e/NPGCapp/result3.RData")
